(async () => {
	...
	const handleTranlater = async e =>{
		...
		const sourseLang = await detectLang(selection)
		...
		document.body.insertAdjacentHTML(
			'beforeend', `<div id="trlr-contaner" class="${theme}"></div>`
		);

		showTranslateContaner(translate, getSelectedRect());
	};

	const getSettings = (set) =>
		new Promise((res, _) =>
			chrome.runtime.sendMessage(set, resp =>
				res(resp ? resp : console.error(resp))
			)
		);

	let { targetLang, theme } = await getSettings('settings');

	const languages = await getSettings('languages');
	
	const waitTime = time => new Promise(resolve => setTimeout(() => resolve(), time));

	const detectLang = (text) => 
		new Promise((res, _) =>
			chrome.i18n.detectLanguage(text, resp => 
				res(resp.isReliable ? resp.languages[0].language : false)
			)
		);
